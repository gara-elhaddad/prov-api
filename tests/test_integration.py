...
def output_file_objs(units):
    objs = [
        ...
        omcore.File(
            id=f"{ID_PREFIX}/{uuid4()}",
            content="File generated by a simulation",
            format=omcore.ContentType(name="application.vnd.nwb.nwbn+hdf"),
...
@pytest.mark.skipif(not have_kg_connection, reason=no_kg_err_msg)
def test_get_visualisation(visualisation_obj, environment_obj, software_version_objs):
    token = kg_client._kg_client.token_handler.get_token()
...
@pytest.mark.skipif(not have_kg_connection, reason=no_kg_err_msg)
def test_patch_visualisation(visualisation_obj):
    token = kg_client._kg_client.token_handler.get_token()
...
@pytest.mark.skipif(not have_kg_connection, reason=no_kg_err_msg)
def test_post_simulation(person_obj, output_file_objs, software_version_objs,
    ...
    data = {
        ...
        "output": [
            {
                "description": output_file_objs[1].content,
